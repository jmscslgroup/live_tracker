...
right_yaw = 90.0
acc_speed = 20
side_street_thresh = 0.02   # Around 200 meters, in lat/long degrees
acc_status = None
...
class LiveTracker:
    def __init__(self):
        ...
        rospy.Subscriber(lat_topic, Float64, lat_callback)
        rospy.Subscriber(lon_topic, Float64, lon_callback)
...
#         rospy.Subscriber(right_yaw_topic, Float64, right_yaw_callback)
        ...
        self.n_side_pub = rospy.Publisher('/debug/n_side', Int16, queue_size=10)
        self.n_true_pub = rospy.Publisher('/debug/n_true', Int16, queue_size=10)
        self.dist_west_pub = rospy.Publisher('/debug/dist_west', Float64, queue_size=10)
        self.dist_east_pub = rospy.Publisher('/debug/dist_east', Float64, queue_size=10)
    ...
    def loop(self):
        while not rospy.is_shutdown():
            try:
                ...
                last_lons.pop()
                last_lons.insert(0, lon)
                last_lats.pop()
                last_lats.insert(0, lat)
                wb, n_true, n_side, dist_east, dist_west = is_wb(last_positions, last_lons, last_lats)
                self.is_wb_pub.publish(wb)
                self.n_true_pub.publish(n_true)
                self.n_side_pub.publish(n_side)
                self.dist_west_pub.publish(dist_west)
                self.dist_east_pub.publish(dist_east)
                gps = getGPSResultStr()
                can = getCANResultStr()
